...
if (filter_input(INPUT_POST, "btnGravar", FILTER_SANITIZE_STRING)) {
    if (filter_input(INPUT_GET, "id", FILTER_SANITIZE_NUMBER_INT)) {
        $password = filter_input(INPUT_POST, "txtSenha", FILTER_SANITIZE_STRING);
        

        if ($usuarioController->AlterarSenha($password, $id)) {

            ?>
            <script>
                document.cookie = "msg=1";
                document.location.href = "?pagina=alterarsenha&id=<?= $id ?>";
            </script>
            <?php
        } else {
